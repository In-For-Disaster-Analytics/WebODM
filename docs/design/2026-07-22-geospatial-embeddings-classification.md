...
**Implementing** — passed a 5-reviewer spec-review pass (Decisions 26-31); implementation underway. See Decision 38 for this increment's real-vs-stub scope (first real frontend component, `EmbeddingsPanel.jsx`, task-detail panel only; a new `GET /sites` endpoint; build tooling not verified to actually run in this environment).
