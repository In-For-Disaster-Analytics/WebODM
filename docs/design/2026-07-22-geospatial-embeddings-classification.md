...
**Verified so far**: settings.py syntax-checked (`py_compile`); the exact assertion source, its call site, and the Django-2.2/psycopg2-2.9 incompatibility itself were confirmed against Django's real `2.2.27` source and the upstream bug tracker, not assumed. **Not yet verified**: a live container restart with this patch applied — Docker Desktop wasn't running locally to test against a real Postgres connection at fix time. Needs a real restart on the remote deployment to confirm the crash loop is actually resolved before calling this done.
