...
**Not yet done, stated plainly**: the map-based tile picker UI itself (the actual "Tile Selection UI: Map, Not a Flat Thumbnail Grid" section's visual design) — `GET .../tiles` is real and callable, but `EmbeddingsPanel.jsx`'s "Label a Sample" section still takes typed `z/x/y` text; only its site selector was added this increment. `TaskLabelsImportGeoJSONView` (GeoJSON label import) remains an explicit stub, untouched. No real end-to-end test has been run yet against a live Label Studio instance for any of this — `py_compile`/webpack build both pass, but the actual webhook round-trip (Label Studio → real annotation → real webhook call → real `labels` row) has not been exercised live.
